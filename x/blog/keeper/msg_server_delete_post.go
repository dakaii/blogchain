package keeper

import (
	"context"
	"fmt"

	"blogchain/x/blog/types"

	errorsmod "cosmossdk.io/errors"
	sdk "github.com/cosmos/cosmos-sdk/types"
	sdkerrors "github.com/cosmos/cosmos-sdk/types/errors"
)

func (k msgServer) DeletePost(ctx context.Context, msg *types.MsgDeletePost) (*types.MsgDeletePostResponse, error) {
	if _, err := k.addressCodec.StringToBytes(msg.Creator); err != nil {
		return nil, errorsmod.Wrap(err, "invalid creator address")
	}

	// Get the existing post
	post, err := k.GetPost(ctx, msg.Id)
	if err != nil {
		return nil, errorsmod.Wrap(err, "post not found")
	}

	// Check if the message sender is the post creator
	if post.Creator != msg.Creator {
		return nil, errorsmod.Wrap(sdkerrors.ErrUnauthorized, "only the post creator can delete the post")
	}

	// Check if post is already deleted
	if post.Deleted {
		return nil, errorsmod.Wrap(sdkerrors.ErrInvalidRequest, "post is already deleted")
	}

	sdkCtx := sdk.UnwrapSDKContext(ctx)

	// Soft delete the post
	post.Deleted = true
	post.DeletedAt = sdkCtx.BlockTime().Unix()

	// Save the deleted post
	if err := k.SetPost(ctx, post); err != nil {
		return nil, errorsmod.Wrap(err, "failed to delete post")
	}

	// Emit event
	sdkCtx.EventManager().EmitEvent(
		sdk.NewEvent(
			"post_deleted",
			sdk.NewAttribute("id", fmt.Sprintf("%d", post.Id)),
			sdk.NewAttribute("creator", post.Creator),
			sdk.NewAttribute("deleted_at", fmt.Sprintf("%d", post.DeletedAt)),
		),
	)

	return &types.MsgDeletePostResponse{}, nil
}